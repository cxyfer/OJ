...
def solve():
    ...
    B = [list(accumulate(range(CYCLE - 1), lambda x, _: pow(x, 3, MOD), initial=x)) for x in A]

    op = lambda x, y: [(a + b) % MOD for a, b in zip(x, y)]
    e = [0] * CYCLE
    def mapping(f, data):
        if f == 0:
            return
        ...
        for i in range(CYCLE - 1):
            data[i] = data[(i + f) % CYCLE] % MOD
        data[CYCLE - 1] = head
    composition = lambda f, g: (f + g) % CYCLE
    id_ = 0
    seg = LazySegTree(op, e, mapping, composition, id_, B)
