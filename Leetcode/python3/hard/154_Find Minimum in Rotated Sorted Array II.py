...
由於最小值一定在 nums[-1] 左側或是 nums[-1] 本身，我們正是利用這個性質來透過與 nums[-1] 的比較來判斷最小值的位置，而在二分的過程中，right + 1 也滿足這個性質，因此可以改成與 nums[right + 1] 的比較。
那麼當遇到與 nums[mid] == nums[right + 1] 時，我們就可以將 right 向左移動一格。
