...
def solve():
    ...
    for qid in range(m):
        ...
        points.append((a - 1, b - 1, 0, qid, 1))
        points.append((a - 1, d, 0, qid, -1))
        points.append((c, b - 1, 0, qid, -1))
        points.append((c, d, 0, qid, 1))
    ...
    def cdq(left: int, right: int) -> None:
        ...
        for j in range(mid + 1, right + 1):
            y, op = points[j][1], points[j][2]
            while i <= mid and points[i][1] <= y:
                cnt += points[i][2]
                i += 1
            if op == 0:
                assert len(points[j]) == 5
                qid = points[j][3]
                factor = points[j][4]
                ans[qid] += cnt * factor
