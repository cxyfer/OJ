...
需要注意在枚舉區間時，不要重新建立長度為 n 的陣列來維護出現次數，這樣會使得複雜度變成 O(n^2)，應該使用 map 或者 set 來維護出現過的元素，並在每次枚舉區間結束後清空（但這種寫法會有額外的時間開銷，本題仍會 TLE）；或是使用同一個陣列，但維護一個 touched 陣列，紀錄被修改過的元素，方便以 O(B) 清空。
