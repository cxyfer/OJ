...
令 s[i] = (s[i - 1] + a[i]) % p，即對前綴和進行取模運算。
對於固定的右端點 r，我們需要找到一個左端點 l，使得 (s[r + 1] - s[l]) % p 最大。

那麼可以考慮 sl = s[l] 和 sr = s[r + 1] 的大小關係，有兩種情況：
- sl <= sr，此時可以選擇 sl <= sr 的 l，且 sl 越小越好
- sl > sr，此時可以選擇 sl > sr 的 l，且 sl 越小越好

使用一個有序容器維護所有的 sl，並在 sl 上二分查找即可。
"""
...
def solve():
    ...
    s = [0] + list(accumulate(A, func=lambda x, y: (x + y) % p))
    ans = (float('-inf'), 0, 0)
    ...
    for j in range(1, n + 1):
        ...
        if sl[0][0] <= sj:
            si, i = sl[0]
            ans = max(ans, (sj - si, i, j - 1))

        idx = sl.bisect_right((sj, float('inf')))
        if idx < len(sl):
            si, i = sl[idx]
            ans = max(ans, (sj - si + p, i, j - 1))

        idx = sl.bisect_left((sj, float('inf')))
        if idx == len(sl) or sl[idx][0] != sj:
            sl.add((sj, j))

    val, i, j = ans
    assert (s[j + 1] - s[i]) % p == val
    print(i, j, val)
