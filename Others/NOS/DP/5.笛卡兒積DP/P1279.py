"""
P1279 字串距离
https://www.luogu.com.cn/problem/P1279
"""
s = input()
t = input()
K = int(input())
n, m = len(s), len(t)


def solve1(s: str, t: str):  # Memoization (MLE)
    from functools import cache
    import sys
    sys.setrecursionlimit(int(5e3))

    @cache
    def f(i: int, j: int) -> int:
        if i < 0:
            return (j + 1) * K
        if j < 0:
            return (i + 1) * K
        return min(f(i - 1, j) + K,
                   f(i, j - 1) + K,
                   f(i - 1, j - 1) + abs(ord(s[i]) - ord(t[j])))
    print(f(n - 1, m - 1))

def solve2(s: str, t: str):  # 查表法
    f = [[0] * (m + 1) for _ in range(n + 1)]
    for i in range(n + 1):
        f[i][0] = i * K
    for j in range(m + 1):
        f[0][j] = j * K
    for i in range(1, n + 1):
        for j in range(1, m + 1):
            f[i][j] = min(f[i - 1][j] + K,
                          f[i][j - 1] + K,
                          f[i - 1][j - 1] + abs(ord(s[i - 1]) - ord(t[j - 1])))
    print(f[n][m])

def solve3(s: str, t: str):  # 刷表法
    s = " " + s + " "
    t = " " + t + " "
    f = [[float("inf")] * (m + 2) for _ in range(n + 2)]
    f[0][0] = 0
    for i in range(n + 1):
        for j in range(m + 1):
            f[i + 1][j] = min(f[i + 1][j], f[i][j] + K)
            f[i][j + 1] = min(f[i][j + 1], f[i][j] + K)
            f[i + 1][j + 1] = min(f[i + 1][j + 1], f[i][j] + abs(ord(s[i + 1]) - ord(t[j + 1])))
    print(f[n][m])

# solve1(s, t)
solve2(s, t)
# solve3(s, t)