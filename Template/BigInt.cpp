...
public:
    ...
    BigInt operator/(const BigInt& divisor) const {
        ...
        res.ln = ln;  // Start with maximum possible length
        res.v.resize(res.ln, 0);  // Initialize with zeros
        ...
        for (int i = ln - 1; i >= 0; i--) {
            // Shift current and add new digit
            current.v.insert(current.v.begin(), v[i]);
            while (current.ln > 1 && current.v[current.ln - 1] == 0) current.ln--;
            current.v.resize(current.ln);
            
            // Find the largest digit q such that divisor * q <= current
            int q = 0;
            ...
            res.v[i] = q;
            current = current - (divisor_abs * BigInt(q));
        }
