...
"""
基於模仿的 sortedcontainers 的 SortedList，實現 SortedDict
...
class SortedDict(dict):
    ...
    def __repr__(self):
        return f"SortedDict({super().__repr__()})"

    def update(self, *args, **kwargs):
        if args:
            if len(args) > 1: raise TypeError("update expected at most 1 arguments")
            other = args[0]
            if isinstance(other, dict):
                for k, v in other.items(): self[k] = v
            elif hasattr(other, 'keys'): # Mapping other than dict
    ...
    def popitem(self, index=-1):
        """移除並返回位於指定 index 的 (key, value) 對"""
        key = self._list.pop(index)
        val = super().pop(key)
        return key, val

    def peekitem(self, index=-1):
        """返回位於指定 index 的 (key, value) 對，但不移除"""
    ...
    def keys(self):
        """直接返回內部的 SortedList，支援索引操作"""
        return self._list

    def values(self):
        for k in self._list: yield self[k]

    def items(self):
        for k in self._list: yield k, self[k]

    # 代理 SortedList 的方法
    def bisect_left(self, key): return self._list.bisect_left(key)
    def bisect_right(self, key): return self._list.bisect_right(key)
    def index(self, key): return self._list.index(key)